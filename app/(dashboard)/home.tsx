...
const StatCard = ({ icon, bg, count, label, badge }: any) => (
    <View className={`rounded-2xl p-4 flex-1 mr-3 ${bg}`} style={{ minWidth: 100 }}>
...
const ActionIcon = ({ iconName, iconColor, bg, label }: any) => (
    <TouchableOpacity className="items-center mb-6 w-1/4">
        <View className={`w-14 h-14 rounded-2xl items-center justify-center mb-2 ${bg}`}>
            <Ionicons name={iconName} size={24} color={iconColor} />
        </View>
        <Text className="text-[11px] text-center text-gray-600 font-medium w-16">{label}</Text>
    </TouchableOpacity>
);

const ActivityRow = ({ iconName, iconColor, iconBg, title, subtitle, status, statusBg, statusColor }: any) => (
            ...
            >
                ...
                <View className="px-5 mb-6">
                    ...
                    <View className="flex-row flex-wrap justify-between">
                        <ActionIcon iconName="add-circle" iconColor="#3B82F6" bg="bg-[#EFF6FF]" label="Request Service" />
                        <ActionIcon iconName="cloud-upload" iconColor="#8B5CF6" bg="bg-[#F5F3FF]" label="Upload Docs" />
                        <ActionIcon iconName="calculator" iconColor="#10B981" bg="bg-[#ECFDF5]" label="GST Calculator" />
                        <ActionIcon iconName="headset" iconColor="#F43F5E" bg="bg-[#FFF1F2]" label="Help Support" />

                        <ActionIcon iconName="bar-chart" iconColor="#EAB308" bg="bg-[#FEFCE8]" label="My Reports" />
                        <ActionIcon iconName="ellipsis-horizontal" iconColor="#64748B" bg="bg-[#F8FAFC]" label="View More" />
                ...
                <View className="px-5 mb-8">
                    <View className="bg-primary rounded-2xl p-5 shadow-sm overflow-hidden h-36 border border-blue-400">
                        ...
                        <Text className="text-white font-bold text-lg mb-1">Tax Season Sale!</Text>
                        <Text className="text-blue-100 text-xs mb-4 w-2/3 leading-4">Get 20% off on premium assisted filing plans.</Text>
                        <TouchableOpacity className="bg-white rounded-full self-start px-4 py-1.5 shadow-sm">
                            <Text className="text-primary font-bold text-xs flex flex-row items-center">Upgrade Now</Text>
                        </TouchableOpacity>
                    </View>
                </View>
                ...
                <View className="px-5 mb-4">
                    ...
                    <ActivityRow
                        iconName="document-text" iconColor="#3B82F6" iconBg="bg-[#EFF6FF]"
                        title="GST Filing Submitted" subtitle="March 2024 Return"
                        status="Processing" statusBg="bg-[#EFF6FF]" statusColor="text-blue-600"
                    />
                    <ActivityRow
                        iconName="briefcase" iconColor="#F97316" iconBg="bg-[#FFF8F0]"
                        title="New Service Request" subtitle="Annual Audit Service"
                        status="Pending" statusBg="bg-[#FFF8F0]" statusColor="text-orange-500"
                    />
                    <ActivityRow
                        iconName="shield-checkmark" iconColor="#10B981" iconBg="bg-[#F0FDF4]"
                        title="Document Verified" subtitle="PAN Card & Aadhar"
                        status="Success" statusBg="bg-[#F0FDF4]" statusColor="text-emerald-500"
                    />
                </View>
